...
document.addEventListener("DOMContentLoaded", () => {
  ...
  updateSplashScreen();
  // Ensure playArea has correct class for result presence on load
  if (typeof updatePlayAreaResultClass === 'function') updatePlayAreaResultClass();
});
...
function startRound() {
  ...
  roundKits = shuffleArray(availableKits).slice(0, roundSize);
...
function loadImage() {
  ...
  // clear result class when loading a new image
  if (typeof updatePlayAreaResultClass === 'function') updatePlayAreaResultClass();
  document.getElementById("newRoundBtn").style.display = "none";
  document.getElementById("gameSettingsBtn").style.display = "none";
  
  // Re-enable and show the reveal button for new image
  const revealBtn = document.getElementById("revealBtn");
  revealBtn.disabled = false;
  revealBtn.classList.remove("disabled");
  revealBtn.style.display = "inline-block";
  
  // Re-enable and show the next button for new image
  const nextBtn = document.getElementById("nextBtn");
  nextBtn.disabled = false;
  nextBtn.classList.remove("disabled");
  nextBtn.style.display = "inline-block";
  
  // Reset typing mode controls
  const guessInput = document.getElementById("guessInput");
  const submitGuessBtn = document.getElementById("submitGuessBtn");
  ...
  if (guessInputWrapper) {
    ...
    guessInputWrapper.style.display = 'flex'; // Reset display
  }
...
function revealAnswer(isCorrect = null, isPartial = false) {
  ...
  if (systemEntry) {
    ...
    if (gameMode === 'typing' && isCorrect !== null) {
      ...
      if (stampOverlay) {
        ...
        if (isCorrect && isPartial) {
          ...
          stampOverlay.innerHTML = `<div class="stamp-text"><span class="stamp-icon">✗</span> WRONG</div>${scoreHtml}`;
        }
    ...
    // mark that we now have a result so CSS can adjust spacing without :has()
    if (typeof updatePlayAreaResultClass === 'function') updatePlayAreaResultClass();
  } else {
...
function nextImage() {
  ...
  // In typing mode, if user hasn't guessed yet (input not disabled), count as incorrect
  if (gameMode === 'typing') {
    const guessInput = document.getElementById("guessInput");
...
function checkAnswer(userGuess) {
  ...
  const hasMatch = correctAnswers.some(answer => {
    ...
    // 3. Base model matching - extract base from user guess and check if it matches
    ...
    // 3b. Reverse base model matching - check if guess matches the base of the answer
    // This handles guessing "T-72" when answer is "T-72BM"
    const baseAnswer = extractBaseModel(normalizedAnswer);
    if (baseAnswer !== normalizedAnswer && baseAnswer.length >= 2) {
      if (normalizedGuess === baseAnswer) {
        return true; // Full match, not partial
      }
    }
    ...
    if (guessWords.length === 1) {
      ...
      return answerWords.some(answerWord => {
        ...
        // Prefix match for longer words (e.g., "alliga" matches "alligator")
        // Require at least 5 chars AND prefix is at least 60% of the full word
        if (guessWord.length >= 5 && 
            answerWord.startsWith(guessWord) && 
            guessWord.length / answerWord.length >= 0.6) {
          return true;
        }
        return false;
      });
...
function updateControlsForMode() {
  ...
  if (gameMode === 'typing') {
    ...
    // Add class for input+Next button styling
    if (controls) controls.classList.add("solo-next");
    // mirror that state on the stage so CSS can target without :has()
    const stageInner = document.querySelector('.stage-inner');
    const stageBody = document.querySelector('.stage-body');
    if (stageInner) stageInner.classList.add('has-solo-next');
    if (stageBody) stageBody.classList.add('has-solo-next');
  } else {
    revealBtn.style.display = "inline-block";
    if (nextBtn) nextBtn.style.display = "inline-block";
    if (guessInputWrapper) guessInputWrapper.style.display = "none";
    // Remove class in flashcard mode
    if (controls) controls.classList.remove("solo-next");
    // remove mirrored class
    const stageInner2 = document.querySelector('.stage-inner');
    const stageBody2 = document.querySelector('.stage-body');
    if (stageInner2) stageInner2.classList.remove('has-solo-next');
    if (stageBody2) stageBody2.classList.remove('has-solo-next');
  }
}
...
function endRound() {
  ...
  // Hide Next and Reveal buttons when round ends
  document.getElementById("nextBtn").style.display = "none";
  document.getElementById("revealBtn").style.display = "none";
  
  // Remove solo-next class when other buttons appear
  const controls = document.querySelector(".controls");
  if (controls) controls.classList.remove("solo-next");

  // Also remove mirrored 'has-solo-next' state on stage elements so
  // typing-mode layout rules don't remain active and prevent the
  // media from shrinking to accommodate the result/new-round UI.
  const stageInner = document.querySelector('.stage-inner');
  const stageBody = document.querySelector('.stage-body');
  if (stageInner) stageInner.classList.remove('has-solo-next');
  if (stageBody) stageBody.classList.remove('has-solo-next');
  
  // Also hide guess input if in typing mode
  const guessInputWrapper = document.querySelector(".guess-input-wrapper");
  if (guessInputWrapper && gameMode === 'typing') {
    guessInputWrapper.style.display = "none";
  }

  // ensure playArea class reflects updated result content
  if (typeof updatePlayAreaResultClass === 'function') updatePlayAreaResultClass();
}
...
// Keep playArea class in sync with whether a result is shown
function updatePlayAreaResultClass() {
  const playArea = document.querySelector('.playArea');
  const resultEl = document.getElementById('result');
  if (!playArea || !resultEl) return;
  const hasResult = !!(resultEl.innerHTML && resultEl.innerHTML.trim() !== '' && resultEl.textContent !== 'Loading images...');
  if (hasResult) playArea.classList.add('has-result'); else playArea.classList.remove('has-result');
}